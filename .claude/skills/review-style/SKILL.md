---
name: review-style
description: Phase 3 of the three-phase review — per-function style. Spawns review-lead (opus), which fans out one style-auditor (sonnet) per changed file; each auditor spawns one finding-refuter (haiku) per finding; the lead arbitrates survivors into .review/REVIEW-FINDINGS.md. Requires .review/minimal.md to say SOLID. Invoked explicitly; not model-invocable.
...
2. Spawn the `review-lead` agent with: phase `style` and the changed-file list above. It fans out one `style-auditor` per file; every finding faces a `finding-refuter`; the lead weighs survivors against AGENTS.md and the readability litmus test.
