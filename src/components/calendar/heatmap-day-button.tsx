...
const HEATMAP_STOPS = [
  { threshold: 1, color: "#46ec13", textColor: "#0a1208", glow: true },
  ...
  { threshold: 0, color: "#ef4444", textColor: "#0a1208", glow: false },
] as const;
...
export function HeatmapDayButton({ className, day, modifiers, ...props }: DayButtonProps) {
  ...
  if (modifiers[MODIFIER_KEYS.DISABLED] || modifiers[MODIFIER_KEYS.OUTSIDE] || !data) {
    return (
      <DayButton
        mode="display"
        className={className}
...
const DEFAULT_HEATMAP_STYLE: React.CSSProperties = {
