...
export default function AppHeader({ planId, variant = "default", className }: AppHeaderProps) {
  ...
  return (
    <div className={outerStyles}>
      <header
        className={cn(
          "mx-auto flex max-w-[1440px] items-center justify-between px-4 py-3 whitespace-nowrap md:px-10",
          className,
        )}
      >
        <Wordmark />
...
function MobileNav({ items }: { items: NavItem[] }) {
  ...
  if (hasNoLinks) {
    return <div className="md:hidden">{buttonItem && <NavButton item={buttonItem} />}</div>;
  }
